...
export const FORCE_SINGLE_STORE = String(process.env.SUSTORE_SINGLE_STORE || '1').trim() !== '0';

export function getStoreIdFromRequest(req) {
    // Single-store mode is the default for Vercel deployments with one public URL.
    const nodeEnv = String(process.env.NODE_ENV || '').toLowerCase();
    if (FORCE_SINGLE_STORE || nodeEnv === 'production') {
