...
<body>
    ...
    <script type="text/babel" data-type="module">
        ...
        const apiKey = ""; 
        // ¡ATENCIÓN! Reemplaza esto con tu KEY DE PRODUCCIÓN real para que cobres de verdad
        ...
        const defaultSettings = {
            ...
            aboutUsText: "Somos una empresa dedicada a traer la mejor tecnología al mejor precio."
        };

        // --- COMPONENTES AUXILIARES (DEFINIDOS ANTES DE APP) ---
            ...
            <div className="fixed inset-0 bg-[#020617]/95 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                {message === 'MP' ? (
                    <>
                        ...
                        <p className="text-white text-xl font-bold tracking-widest animate-pulse">ABRIENDO MERCADO PAGO</p>
        ...
        const WelcomeModal = ({ code, onClose }) => (
            <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-up">
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 border border-cyan-500/50 p-8 rounded-3xl shadow-[0_0_50px_rgba(6,182,212,0.2)] max-w-md w-full text-center relative overflow-hidden">
                    ...
                    <p className="text-slate-300 mb-6 text-sm">Gracias por unirte. Aquí tienes tu regalo exclusivo:</p>
                    <div className="bg-slate-950 border border-dashed border-cyan-500/50 p-5 rounded-xl mb-6 cursor-pointer hover:bg-slate-900 transition group" onClick={() => {navigator.clipboard.writeText(code); alert('¡Copiado!');}}>
                        <p className="text-[10px] text-slate-500 uppercase tracking-widest mb-1 font-bold">Código de Descuento</p>
        ...
        function App() {
            ...
            const [chatMessages, setChatMessages] = useState([{ role: 'system', text: '¡Hola! Soy la IA de Sustore.' }]);
            ...
            const handleGoToMP = async () => {
                if (!checkoutData.address || !checkoutData.city || !checkoutData.zipCode || !checkoutData.transferHolder) return alert("Por favor completa todos los campos de envío.");
                if (!/^\d{4}$/.test(checkoutData.zipCode)) return alert("El CP debe ser un número de 4 dígitos válido.");
                ...
                try {
                    ...
                    // 4. REDIRECCIÓN DIRECTA (Obligatoria)
                    if (preference.url) {
                        window.location.href = preference.url;
                    } else {
                        throw new Error("El servidor no devolvió un link de pago válido.");
                    }
                    
                } catch (error) {
                    console.error("CRITICAL PAYMENT ERROR:", error);
                    alert("⛔ ERROR DE PAGO:\n" + error.message + "\n\nVerifica tu consola para más detalles.");
                    setIsLoading(false);
                }
            };
            ...
            const handleRecoverYes = () => {
                ...
                // Restaurar datos del formulario si existen en la orden guardada
                if (pendingOrderRecover.checkoutData) {
                    setCheckoutData(pendingOrderRecover.checkoutData);
                } else {
                    // Fallback para órdenes viejas: Intentar rellenar lo que tengamos
                    setCheckoutData(prev => ({ ...prev, transferHolder: pendingOrderRecover.userName }));
                }
            ...
            const validateCoupon = () => {
                ...
                if (!found) return alert("Cupón inválido");
            ...
            const saveCoupon = async () => { if(!newCoupon.code) return alert("Falta código"); setIsLoading(true); await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'coupons'), { ...newCoupon, usedBy: [] }); setNewCoupon({ code: '', discountPercentage: 7, expirationDate: '', userEmail: '' }); setIsLoading(false); };
            const deleteCoupon = async (id) => { if(!confirm("¿Eliminar cupón?")) return; await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'coupons', id)); };
            ...
            const generateDescription = async () => {
                setAiThinking(true);
                const text = await callGemini(`Descripción corta para ${newProduct.name} (${newProduct.category}).`);
            ...
            const sendChat = async (e) => {
                ...
                const ans = await callGemini(`Eres Sustore AI. Cliente: ${msg}. Corto y útil.`);
                    ...
                    <main className="flex-grow">
                            ...
                            <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-up">
                                <div className="bg-slate-800 border border-yellow-500/50 p-6 rounded-2xl shadow-2xl max-w-md w-full text-center relative overflow-hidden">
                                    ...
                                    <div className="flex gap-3 relative z-10">
                                        ...
                                        <button onClick={handleRecoverYes} className="flex-1 py-3 bg-yellow-600 hover:bg-yellow-500 text-white rounded-xl font-bold transition shadow-lg shadow-yellow-900/20">Sí, continuar</button>
                                    </div>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="animate-fade-up">
                                <div className="relative h-[500px] flex items-center justify-center overflow-hidden bg-slate-950">
                                    ...
                                    <div className="relative z-10 text-center px-4">
                                        <span className="bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 text-xs font-bold px-6 py-2 rounded-full uppercase tracking-[0.2em] mb-6 inline-block backdrop-blur-sm shadow-[0_0_15px_rgba(6,182,212,0.1)]">Tecnología Premium</span>
                                        <h1 className="text-5xl md:text-7xl font-black text-white mb-8 drop-shadow-2xl tracking-tight neon-text">NEXT LEVEL</h1>
                                        <button onClick={() => document.getElementById('products')?.scrollIntoView({behavior:'smooth'})} className="bg-white text-slate-950 px-10 py-4 rounded-full font-bold shadow-[0_0_30px_rgba(255,255,255,0.2)] hover:scale-105 transition duration-300 hover:shadow-[0_0_40px_rgba(255,255,255,0.4)]">EXPLORAR CATÁLOGO</button>
                                    </div>
                                </div>
                            ...
                            <div className="max-w-md mx-auto px-4 py-12 animate-fade-up">
                                ...
                                <div className="bg-white p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] text-slate-900 relative overflow-hidden deposit-card">
                                    ...
                                    <div className="flex gap-2 mb-6">
                                        <input className="flex-1 bg-slate-100 border border-slate-200 p-3 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-purple-500 uppercase" placeholder="CÓDIGO CUPÓN" value={checkoutCoupon} onChange={e => setCheckoutCoupon(e.target.value)}/>
                                    ...
                                    {/* RESUMEN PRODUCTOS */}
                                    <div className="bg-slate-100 p-4 rounded-xl mb-6 max-h-40 overflow-y-auto">
                                        {cart.map((item,i) => (
                                            <div key={i} className="flex justify-between text-sm mb-2 border-b border-slate-200 pb-2 last:border-0 last:pb-0">
                                                <span className="text-slate-700 font-bold">{item.quantity}x {item.product.name}</span>
                                                <span className="text-slate-900 font-mono">${(calculatePrice(item.product.basePrice, item.product.discount)*item.quantity).toLocaleString()}</span>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <input className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-slate-800 focus:bg-white focus:border-cyan-500 outline-none transition" placeholder="Dirección de Envío Completa" value={checkoutData.address} onChange={e => setCheckoutData({...checkoutData, address: e.target.value})}/>
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <input className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-slate-800 focus:bg-white focus:border-cyan-500 outline-none transition" placeholder="CP (4 dígitos)" maxLength="4" value={checkoutData.zipCode} onChange={e => setCheckoutData({...checkoutData, zipCode: e.target.value})}/>
                                        </div>
                        ...
                        {view === 'cart' && (
                            <div className="max-w-2xl mx-auto px-4 py-12 animate-fade-up">
                                ...
                                {cart.length === 0 ? <p className="text-slate-500 text-center py-20 bg-slate-900/50 rounded-3xl border border-dashed border-slate-800">Tu carrito está vacío.</p> : (
                                <div className="space-y-4">
                                    {cart.map((item, i) => (
                            ...
                            <div className="flex items-center justify-center min-h-[80vh] px-4 animate-fade-up">
                                <div className="bg-slate-900 p-10 rounded-3xl shadow-2xl w-full max-w-md border border-slate-800 relative overflow-hidden">
                                    ...
                                    <form onSubmit={view === 'login' ? onLoginSubmit : onRegisterSubmit} className="space-y-4">
                                        ...
                                        {view === 'register' && <input className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl text-white outline-none focus:border-cyan-500 transition" placeholder="Teléfono" value={registerData.phone} onChange={e => setRegisterData({...registerData, phone: e.target.value})}/>}
                                        <input className="w-full bg-slate-950 border border-slate-700 p-4 rounded-xl text-white outline-none focus:border-cyan-500 transition" type="password" placeholder="Contraseña" value={view === 'login' ? loginData.password : registerData.password} onChange={e => view === 'login' ? setLoginData({...loginData, password: e.target.value}) : setRegisterData({...registerData, password: e.target.value})}/>
                                    ...
                                    <button onClick={() => { setView(view === 'login' ? 'register' : 'login'); setAuthError(''); }} className="w-full text-center text-slate-400 text-sm mt-8 hover:text-white transition">{view === 'login' ? '¿No tienes cuenta? Regístrate' : '¿Ya tienes cuenta? Ingresa'}</button>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="max-w-3xl mx-auto px-4 py-12 animate-fade-up">
                                ...
                                <div className="grid sm:grid-cols-2 gap-4 mb-8">
                                    {coupons.filter(c => (c.userEmail === currentUser.email) && !c.usedBy.includes(currentUser.id)).map(c => (
                                        <div key={c.id} className="bg-slate-900 border border-yellow-500/30 p-4 rounded-xl flex justify-between items-center relative overflow-hidden group cursor-pointer" onClick={() => {navigator.clipboard.writeText(c.code); alert("Cupón copiado");}}>
                        ...
                        {view === 'admin' && isAdmin(currentUser?.email) && (
                            <div className="max-w-6xl mx-auto px-4 py-12 animate-fade-up">
                                <div className="flex justify-between items-end mb-8 bg-slate-900 p-6 rounded-2xl border border-slate-800">
                                    <div><h2 className="text-3xl font-bold text-white mb-1">Panel de Control</h2><p className="text-slate-400 text-sm">Gestión Total</p></div>
                                    ...
                                    <div className="lg:col-span-2 space-y-8">
                                        <div className="bg-slate-900 p-6 rounded-2xl shadow-lg border border-slate-800">
                                            ...
                                            <div className="grid md:grid-cols-2 gap-5">
                                                ...
                                                <div className="md:col-span-2"><label className="text-xs text-slate-400 font-bold uppercase ml-1 mb-1 block">Categoría</label><select className="w-full bg-slate-950 border border-slate-700 p-3 rounded-xl outline-none text-white focus:border-cyan-500 transition" value={newProduct.category} onChange={(e) => setNewProduct({...newProduct, category: e.target.value})}><option value="">Seleccionar...</option>{settings.categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></div>
                                        ...
                                        {/* GESTIÓN CUPONES */}
                                        <div className="bg-slate-900 p-6 rounded-2xl shadow-lg border border-slate-800">
                                            <h3 className="font-bold text-white mb-6 flex gap-2 items-center text-lg"><Ticket className="text-yellow-400"/> Crear Cupón</h3>
                                            <div className="grid grid-cols-2 gap-4">
                                                <input className="bg-slate-950 border border-slate-700 p-3 rounded-lg text-white" placeholder="CÓDIGO" value={newCoupon.code} onChange={e => setNewCoupon({...newCoupon, code: e.target.value.toUpperCase()})}/>
                                            ...
                                            <button onClick={saveCoupon} className="w-full mt-4 bg-yellow-600 text-white font-bold py-2 rounded-lg hover:bg-yellow-500">Crear Cupón</button>
                        ...
                        {view === 'users_list' && isAdmin(currentUser?.email) && (
                            <div className="max-w-6xl mx-auto px-4 py-12 animate-fade-up">
                                ...
                                <div className="bg-slate-800 rounded-2xl shadow-lg border border-slate-700 overflow-hidden">
                                    <table className="w-full text-left text-sm"><thead className="bg-slate-900 text-cyan-400 uppercase text-xs font-bold tracking-wider"><tr><th className="p-5">Nombre</th><th className="p-5">Email</th><th className="p-5">DNI</th><th className="p-5">Teléfono</th><th className="p-5">Acción</th></tr></thead><tbody className="divide-y divide-slate-700/50">{users.map(u => (<tr key={u.id} className="hover:bg-slate-700/30 transition"><td className="p-5 font-bold text-white">{u.name}</td><td className="p-5 text-cyan-300">{u.email}</td><td className="p-5 text-slate-300">{u.dni}</td><td className="p-5 text-slate-300">{u.phone}</td><td className="p-5"><button onClick={() => handleEditUser(u)} className="text-blue-400 hover:text-blue-200 p-2 bg-blue-900/20 rounded"><Edit className="w-4 h-4"/></button></td></tr>))}</tbody></table>
                                </div>
                                {editingUser && (
                                    <div className="fixed inset-0 bg-black/80 z-[90] flex items-center justify-center p-4">
                                        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-700 w-full max-w-md">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <input className="w-full bg-slate-950 border border-slate-700 p-3 rounded-lg text-white" value={editingUser.phone} onChange={e => setEditingUser({...editingUser, phone: e.target.value})} placeholder="Teléfono"/>
                                            </div>
                        ...
                        {isMenuOpen && (
                            <div className="fixed inset-0 z-[60] flex justify-end animate-fade-up">
                                ...
                                <div className="relative bg-slate-900 w-72 h-full shadow-2xl flex flex-col border-l border-slate-800">
                                    <div className="p-6 border-b border-slate-800 flex justify-between items-center"><span className="font-bold text-lg text-white tracking-widest">MENÚ</span><button onClick={() => setIsMenuOpen(false)}><X className="text-slate-400 hover:text-white"/></button></div>
                                    ...
                                    {currentUser && <div className="p-6 border-t border-slate-800"><button onClick={() => setShowLogoutConfirm(true)} className="w-full py-3 border border-red-500/20 text-red-400 rounded-xl text-sm font-bold hover:bg-red-500/10 flex items-center justify-center gap-2 transition"><LogOut className="w-4 h-4" /> Cerrar Sesión</button></div>}
                                </div>
                            </div>
                        )}
