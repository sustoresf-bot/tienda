...
import { exchangeOAuthCodeForTokens } from '../../../lib/mercadopago/oauth.js';
import { saveStoreOAuthCredentials } from '../../../lib/mercadopago/store-credentials.js';
...
async function findStateSnapshot(db, state) {
    ...
    const FieldPath = adminSdk.firestore.FieldPath;
    const snap = await db
        .collectionGroup('mercadoPagoOauthStates')
        .where(FieldPath.documentId(), '==', state)
        .limit(2)
        .get();
